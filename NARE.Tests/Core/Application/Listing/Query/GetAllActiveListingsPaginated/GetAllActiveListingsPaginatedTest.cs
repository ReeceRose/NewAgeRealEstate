...
{
    public class GetAllActiveListingsPaginatedTest
    {
        public Mock<IMediator> Mediator { get; }
        public IMapper Mapper { get; }
        public GetAllActiveListingsPaginatedQueryHandler Handler { get; }
        public List<NARE.Domain.Entities.Listing> Listings { get; }

        public GetAllActiveListingsPaginatedTest()
        {
            // Arrange
            Mediator = new Mock<IMediator>();
            Mapper = new Mapper(new MapperConfiguration(cfg => cfg.AddProfile(new MappingProfile())));
            Handler = new GetAllActiveListingsPaginatedQueryHandler(Mediator.Object, Mapper);
        ...
        [InlineData(1, 1, 2)]
        [InlineData(10, 1, 1)]
        public async Task GetAllActiveListingsPaginated_ReturnsExpected(int pageSize, int currentPage, int totalPages)
        {
            // Arrange
            ...
            // Act
            var result = await Handler.Handle(new GetAllActiveListingsPaginatedQuery(paginatedListings.PaginationModel), CancellationToken.None);
            // Assert
